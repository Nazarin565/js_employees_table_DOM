...
form.addEventListener("submit", function(e) {
    ...
    form.querySelectorAll("label").forEach((label)=>{
        ...
        if (!input.value.trim()) {
            pushNotification("error", `${input.name.charAt(0).toUpperCase() + input.name.slice(1)} field can't be empty!`);
            hasErrors = true;
            return;
        ...
        if (input.name === "salary") {
            ...
            newEmployeeData[input.name] = `$${parseFloat(salary.toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;
